"""
APEXEYE LINUX CLIENT — Automatic Master Discovery Module

Implements multi-layer Master endpoint resolution with strict Windows behavioral parity:
...
def get_cache_file_path() -> Path:
    """Determine path to local Master endpoint cache file for Linux client."""
    try:
        from client_linux.app.config import find_env_file
        env_p = find_env_file()
        if env_p:
    ...
    if (Path.cwd() / "client_linux").is_dir():
        return Path.cwd() / "client_linux" / ".apexeye_master_cache.json"
    return cwd_cache
...
class MasterDiscoverer:
    """Multi-tier discovery engine for APEXEYE Master on Linux."""
    ...
    def resolve(
        ...
        """
        Full layered resolution matching Windows client precedence:
