"""
APEXEYE LINUX CLIENT — Configuration Module

Loads Linux Client-specific configuration with deterministic .env discovery,
...
def get_candidate_env_paths() -> List[Path]:
    ...
    Search Order:
      ...
      5. Nested client_linux/.env subdirectories
    """
    ...
    try:
        if sys.argv and sys.argv[0]:
            ...
            if argv_p.name.lower() not in ("pytest", "pytest.exe", "-c"):
                ...
                add_candidate(start_dir / "client_linux" / ".env")
                for parent in start_dir.parents:
                    add_candidate(parent / ".env")
                    add_candidate(parent / "client_linux" / ".env")
    except Exception:
        pass
    ...
    try:
        ...
        add_candidate(cwd / "client_linux" / ".env")
...
_last_linux_resolution_details: LinuxResolutionDetails = LinuxResolutionDetails()


def get_last_linux_resolution_details() -> LinuxResolutionDetails:
    """Retrieve diagnostic details from the most recent configuration resolution."""
    return _last_linux_resolution_details
...
def _resolve_master_configuration(enable_discovery: bool = True) -> Tuple[str, str, Optional[str], str, int]:
    ...
    """
    global _last_linux_resolution_details
    details = LinuxResolutionDetails()
    ...
    if proc_master_url:
        ...
        if enable_discovery:
            try:
                from client_linux.app.discovery import verify_master_endpoint, save_cached_master, master_discoverer
    ...
    _last_linux_resolution_details = details
...
class LinuxClientConfig:
    """Configuration for the APEXEYE Linux Client agent."""

    APP_NAME = "APEXEYE Linux Client"
    VERSION = "0.3.0"
    ...
    def reload(self):
        ...
        self.CLIENT_ID = os.getenv("APEXEYE_CLIENT_ID") or str(uuid.uuid4())
        ...
        # Resolution details
        details = get_last_linux_resolution_details()
        ...
        self.CLIENT_UI_PORT = int(os.getenv("APEXEYE_CLIENT_UI_PORT", "9200"))
        ...
        self.TELEMETRY_INTERVAL = int(
            os.getenv("APEXEYE_CLIENT_TELEMETRY_INTERVAL_SECONDS", "10")
        )
