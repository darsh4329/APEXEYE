"""
APEXEYE LINUX CLIENT — Secure Command Handler (Phase 10)
...
logger = get_logger("apexeye.linux_client.command_handler")
...
class CommandHandler:
    """Handles execution of authenticated, allowlisted administrative commands on Linux."""
    ...
    def handle_command(self, payload: dict) -> dict:
        ...
        try:
            ...
            logger.error("Linux Command %s execution error: %s", cmd_type, exc, exc_info=True)
            return {"success": False, "command_id": cmd_id, "error": str(exc)}
    ...
    def _exec_reauthenticate_agent(self, cmd_id: str) -> dict:
        ...
        logger.warning(
            "Master ordered forced re-authentication for Linux client %s. Invalidating session.",
            dev_id,
        )
    ...
    def _exec_update_monitoring_policy(self, cmd_id: str, params: dict) -> dict:
        ...
        if tel_interval is not None:
            try:
                ...
                if 5 <= val <= 300:
                    ...
                    return {"success": False, "command_id": cmd_id, "error": f"Invalid telemetry_interval {val}. Allowed: 5-300s."}
        ...
        if scan_interval is not None:
            try:
                ...
                if 2 <= val <= 60:
                    ...
                    return {"success": False, "command_id": cmd_id, "error": f"Invalid event_scan_interval {val}. Allowed: 2-60s."}
            except (ValueError, TypeError):
        ...
        if severity is not None:
            ...
            if sev_clean in ("INFO", "WARNING", "ERROR", "CRITICAL"):
                ...
                return {"success": False, "command_id": cmd_id, "error": f"Invalid log_severity '{severity}'."}
