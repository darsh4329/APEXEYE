"""
APEXEYE CLIENT — Communication Module (Phase 2)

HTTP communication with the Master API.
Phase 2 additions:
  ...
  - Graceful handling of Master unavailability
"""
...
from client.app.config import config, is_loopback_url
from client.app.utils.logger import get_logger

logger = get_logger("apexeye.client.communication")

# Maximum items in the offline buffer (prevents unbounded memory growth)
_MAX_BUFFER_SIZE = 50


class MasterConnection:
    """Manages HTTP communication with the APEXEYE Master."""
    ...
    def __init__(self, master_url: str):
        ...
        # Offline buffer: stores (endpoint_path, payload) tuples
        # when Master is temporarily unreachable
    ...
    def set_identity(self, device_id: str | None, token: str | None) -> None:
        ...
        if device_id and token:
            logger.info("Communication identity set for device %s", device_id)
        else:
            logger.info("Communication identity cleared.")
    ...
    def clear_identity(self) -> None:
        ...
        logger.info("Communication identity cleared (session terminated).")
    ...
    def check_master_connectivity(self, tcp_timeout: float = 3.0) -> dict:
        """
        Perform a comprehensive pre-flight connectivity check:
    ...
    def format_diagnostics(self, reason: str = "", details: dict | None = None) -> str:
        ...
        lines = [
            "=" * 60,
            "APEXEYE CLIENT CONNECTIVITY DIAGNOSTIC",
            "=" * 60,
        ...
        if is_loopback:
            lines.extend([
                ...
                f"{self.master_url}",
                "",
                "127.0.0.1 means THIS CLIENT COMPUTER.",
        ...
        lines.extend([
            ...
            "  3. Host firewall on Master PC (Windows Defender Firewall) blocks inbound TCP 9100.",
            ...
            "  Step 1: On Master PC, verify Master is running and listening on 0.0.0.0:9100:",
            "          python -m master.main",
            "  Step 2: On Master PC, find actual active Wi-Fi IPv4 address (ignore VirtualBox 192.168.56.x):",
            "          ipconfig",
            "  Step 3: On Master PC, verify inbound firewall rule for TCP 9100 (PowerShell as Admin):",
            "          New-NetFirewallRule -DisplayName 'APEXEYE Master API' -Direction Inbound -LocalPort 9100 -Protocol TCP -Action Allow -Profile Private,Domain",
            "  Step 4: On Client PC, test port & health check:",
            f"          Test-NetConnection {host} -Port {port}",
            f"          curl {self.master_url}/api/health",
            "  Step 5: In Client .env, configure APEXEYE_MASTER_URL=http://<MASTER-LAN-IP>:9100",
            "=" * 60,
        ])
        return "\n".join(lines)
    ...
    def set_master_url(self, new_url: str, source: str = "rediscovery") -> None:
        ...
        try:
            ...
            pass
        logger.info("Active Master URL updated to %s", self.master_url)
    ...
    def attempt_rediscovery(self) -> bool:
        ...
        """
        try:
            from client.app.discovery import master_discoverer, verify_master_endpoint
            # 1. Try LAN discovery
    ...
    def _request_with_retry(
        ...
        Retries on connection errors (status 0) only — not on HTTP errors
        like 401 or 400 which indicate a real server response.
        ...
        for attempt in range(max_retries + 1):
            ...
            # Success or real HTTP error (server responded) — don't retry
            if status != 0:
                return status, resp

            # Connection failure: attempt automatic rediscovery
            if attempt == 0 or (attempt == 1):
                if self.attempt_rediscovery():
                    # Master found at new IP: retry immediately with preserved credentials
                    status, resp = self._request(method, path, data, authenticated)
                    if status != 0:
                        return status, resp
    ...
    def flush_buffer(self) -> None:
        ...
        Called when a successful send indicates Master is back online.
        """
    ...
    def _send_with_buffer(
        ...
        """
        Send a payload to Master. On success, also flush any buffered data.
        On connection failure, buffer the payload for later.
        """
        status, resp = self._request_with_retry("POST", path, payload)

        if status in (200, 201):
            # Success — also try to flush any buffered items
            self.flush_buffer()
            return True
        elif status == 0:
            # Master unreachable — buffer for later
            self._buffer_payload(path, payload)
            return False
        else:
            # Real error (400/401) — don't buffer bad payloads
            logger.warning(
    ...
    def register_self(self, device_info: dict) -> tuple[int, dict]:
        ...
        """
        logger.info("Requesting registration with Master at %s …", self.master_url)
    ...
    def authenticate(self, device_id: str, token: str, device_info: dict | None = None) -> tuple[int, dict]:
        ...
        """
        logger.info("Authenticating device %s with Master …", device_id)
        payload = {"token": token}
        if device_info:
    ...
    def send_telemetry(self, payload: dict) -> bool:
        ...
        POST /api/telemetry
        Buffers on failure for retry when Master returns.
        """
    ...
    def send_heartbeat(self, client_status: str = "running") -> bool:
        ...
        POST /api/heartbeat
        Heartbeats are NOT buffered — stale heartbeats are useless.
        """
        if not self.is_authenticated:
        ...
        if status in (200, 201):
            # Heartbeat success = Master is alive → flush buffer
            self.flush_buffer()
            return True
    ...
    def get_firewall_policy(self) -> dict | None:
        ...
        Returns the policy dict or None on failure.
        """
        if not self.is_authenticated:
            return None
