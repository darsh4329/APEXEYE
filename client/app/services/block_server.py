"""
APEXEYE FIREWALL — Block Server & Network Interception Responder
...
logger = get_logger("apexeye.client.block_server")
...
def _graceful_close(sock: socket.socket) -> None:
    ...
    On Windows (Winsock), closing a stream socket abruptly without FIN/drain
    emits a TCP RST segment. When modern browsers (Chrome/Edge/Firefox) receive
    TCP RST, they abort the stream and display ERR_CONNECTION_RESET instead of
    rendering the received HTTP response.
    """
    try:
    ...
    def start(self) -> tuple[bool, Optional[str]]:
        ...
        for s in self._http_socks:
            t = threading.Thread(
                ...
                args=(s,),
                name=f"ApexEye-BlockServer-HTTP-{s.family.name}",
                daemon=True,
            )
        ...
        for s in self._https_socks:
            t = threading.Thread(
                ...
                args=(s,),
                name=f"ApexEye-BlockServer-HTTPS-{s.family.name}",
                daemon=True,
            )
            self._threads.append(t)
            t.start()
        ...
        logger.info(
            "APEXEYE Block Server started successfully on %s and [%s] (HTTP:%d, HTTPS:%d, %d listeners)",
