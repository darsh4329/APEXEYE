"""
APEXEYE CLIENT — Local Web UI & Client Dashboard Server

Runs on the Client PC (http://127.0.0.1:9200):
- If unauthenticated: serves the Client Authentication Screen.
- If authenticated: serves the dedicated Client Dashboard with local metrics.
...
logger = get_logger("apexeye.client.ui")
...
_last_metrics_sample = None
_last_metrics_time = 0.0
_METRICS_CACHE_TTL = 1.5
...
def create_client_ui_app(
    ...
    """Build the Flask application for the local Client UI / Dashboard."""
    template_dir = os.path.join(os.path.dirname(__file__), "templates")
    app = Flask("apexeye_client_ui", template_folder=template_dir)
    ...
    def index():
        ...
        return render_template(
            ...
            device_name=os_info.get("hostname", "Windows PC"),
            os_name=os_info.get("os_full", "Windows"),
            hostname=os_info.get("hostname", "localhost"),
            ip_address=os_info.get("ip_address", "127.0.0.1"),
            master_url=config.master_url,
            ...
            current_app="Desktop / Active Window",
        )
    ...
    def get_metrics():
        # Authenticate live on EVERY request — NEVER cached
        ...
        global _last_metrics_sample, _last_metrics_time
        import time
        now = time.time()
        with _metrics_lock:
            if _last_metrics_sample is not None and (now - _last_metrics_time) < _METRICS_CACHE_TTL:
                return jsonify(_last_metrics_sample)
    ...
    def get_firewall_status():
        """
        Read-only firewall status endpoint.
        Returns the current firewall state and readiness diagnostics from the local FirewallAgent.
        No management controls — state is controlled only by Master.
        """
        firewall_state = {
            "policy_active": False,
...
def start_client_ui_server(
    ...
    thread = threading.Thread(target=_run, daemon=True, name="ApexEye-ClientUI")
    thread.start()
    logger.info("Client UI Dashboard started on http://127.0.0.1:%s", port)
    return app, thread
