"""
APEXEYE CLIENT — Entry Point (Phase 2)
...
def open_login_page(url: str) -> bool:
    ...
    Gracefully handles headless environments without raising exceptions.
    """
    logger = get_logger("apexeye.client")
    try:
        import webbrowser
...
def prompt_client_authentication_async(auth: ClientAuth, authenticated_event: threading.Event, stop_event: threading.Event) -> None:
    ...
    print("\n" + "=" * 60)
    print("APEXEYE CLIENT — TERMINAL PAIRING PROMPT")
    print("=" * 60)
    print("Enter credentials below OR open http://127.0.0.1:9200 in your browser.")
    print("=" * 60 + "\n")
    ...
    print("Terminal authentication attempts ended. Use http://127.0.0.1:9200 in your browser to pair.\n")
...
def main() -> None:
    ...
    print("\n" + "=" * 60)
    print("APEXEYE CLIENT CONFIGURATION")
    print("=" * 60)
    ...
    if config.is_localhost_target:
        ...
        print("\nFor a physical two-PC LAN deployment, ensure Master is running on 0.0.0.0:9100 on the same Wi-Fi network,")
    ...
    def on_authenticated(device_id: str, token: str):
        ...
        logger.info("Client authenticated callback received for device %s", device_id)
    ...
    while not stop_event.is_set():
        ...
        current_agent = Agent(conn, firewall_agent=firewall_agent)
        current_agent.start()

        logger.info("=" * 60)
        logger.info("APEXEYE Client Agent is running.")
        logger.info("Client Dashboard : http://127.0.0.1:%s", client_ui_port)
        logger.info("Press Ctrl+C to stop.")
        logger.info("=" * 60)

        # Block until interrupted or forced disconnect
        try:
            current_agent.wait()
        except KeyboardInterrupt:
        ...
        # Stop and join workers cleanly from main thread
        try:
            if current_agent and hasattr(current_agent, "stop"):
                current_agent.stop(timeout=5.0)
        except Exception as exc:
            logger.debug("Error stopping agent workers from main thread: %s", exc)
        current_agent = None
        ...
        if disconnect_event.is_set():
            logger.warning("Agent execution stopped due to disconnect event. Resetting lifecycle to WAITING_FOR_PAIRING.")
            authenticated_event.clear()
            disconnect_event.clear()
            continue
        else:
    ...
    logger.info("APEXEYE Client Agent shut down cleanly.")
